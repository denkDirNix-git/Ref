...
type
  ...
  TSIHShiftState = set of (
    ssShift_ = Ord(System.Classes.ssShift),
    ssAlt_   = Ord(System.Classes.ssAlt),
    ssCtrl_  = Ord(System.Classes.ssCtrl),
    ssWin    = 32);
...
begin
  ...
  if (ScanCode and $100) <> 0 then
    Include(ShiftState, ssShift_);
  // Control
  if (ScanCode and $200) <> 0 then
    Include(ShiftState, ssCtrl_);
  // Alt
  if (ScanCode and $400) <> 0 then
    Include(ShiftState, ssAlt_);
...
begin
  if ssShift_ in ShiftState then
    Shifts := GetVirtualKey(VK_SHIFT, Press, Release)
  else
    Shifts := nil;

  if ssCtrl_ in ShiftState then
    Ctrls := GetVirtualKey(VK_CONTROL, Press, Release)
  else
    Ctrls := nil;

  if ssAlt_ in ShiftState then
